...
_TOOL_NAME_MAP = {
    ...
    "improvement_queue": "manage_queue",
    "queue_improvement": "manage_queue",
    "add_improvement": "manage_queue",
...
def _parse_stepfun_tool_call(tool_name: str, body: str) -> Optional[ToolBlock]:
    """Parse StepFun native tool-call tokens into an AD ToolBlock."""
...
def parse_tool_blocks(text: str, skip_fenced: bool = False) -> List[ToolBlock]:
    ...
    if not skip_fenced:
        for m in _TOOL_BLOCK_RE.finditer(text):
            ...
            tag, content = call
            if not content:
                # Empty fences have nothing to run (bash, python, ...).
                continue
